	...
	if (sd.teamNum <= 4) {
		...
		for (int player = 0, chara = 0; player < 4; player++) {
			...
			teamBox[player].setSize(
				100, imag(sd.chara[chara].image.box.size()) + SCORE_NUM * (sd.font.chara.size + space));
		...
		for (int playerNo = 4, charaNo = 0; playerNo < sd.teamNum; playerNo++) {
			...
			teamBox[playerNo].setSize(
				100, imag(sd.chara[charaNo].image.box.size()) + SCORE_NUM * (sd.font.chara.size + space));
...
void CountUp::draw() {
	Scene::draw();
	DrawFormatStringToHandle(real(sd.screen.center()) - 90, sd.obj.upperFrame.box.bottom() + 10,
