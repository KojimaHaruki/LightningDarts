...
void cTimer::update() {
	if (pause) return; 
	mLapseTime.mt = GetNowCount() - startTime; 
	mLapseTime = time(mLapseTime.mt); 
	return;
}
...
int cTimer::drawLapseTime(int x, int y, unsigned int color, int mode) {
	switch (mode) {
	case Mode::HMSmS: 
		DrawFormatString(x, y, color, "%02d:%02d:%02d.%03d", mLapseTime.h, mLapseTime.m, mLapseTime.s, mLapseTime.ms); break;
	case Mode::MSmS: 
		DrawFormatString(x, y, color, "%02d:%02d.%03d", 60 * mLapseTime.h + mLapseTime.m, mLapseTime.s, mLapseTime.ms); break;
	case Mode::SmS: 
		DrawFormatString(x, y, color, "%6.3lf", mLapseTime.t); break;
	case Mode::mS: 
		DrawFormatString(x, y, color, "%d ms", mLapseTime.mt); break;
	case Mode::HMS: 
		DrawFormatString(x, y, color, "%02d:%02d:%02d", mLapseTime.h, mLapseTime.m, mLapseTime.s); break;
	case Mode::MS: 
		DrawFormatString(x, y, color, "%02d:%02d", 60 * mLapseTime.h + mLapseTime.m, mLapseTime.s); break;
	case Mode::S: 
		DrawFormatString(x, y, color, "%d s", (int)mLapseTime.t); break;
	case Mode::HM: 
		DrawFormatString(x, y, color, "%02d:%02d", mLapseTime.h, mLapseTime.m); break;
	default: return -1;
	} return 0;
}
