...
void Home::draw() {
    Scene::draw();
    DrawStringToHandle(sd.screen.center().x() - 3 * sd.font.xl.size,
        sd.screen.center().y() - sd.font.xl.size / 2, "Lightning Darts", sd.color.touch, sd.font.xl.handle);
    DrawGraph(sd.screen.center().x() - 56, 11 * sd.screen.center().y() / 8 - 12, sd.ctrl.start.key.image.handle, TRUE);
    int y = 11 * sd.screen.center().y() / 8 - sd.font.m.size / 2;
    DrawStringToHandle(11 * sd.screen.center().x() / 16, y, "Press", sd.color.w, sd.font.m.handle);
    DrawStringToHandle(sd.screen.center().x() - 30, y, "space", sd.color.w, sd.font.m.handle);
    DrawStringToHandle(19 * sd.screen.center().x() / 16, y, "to start ...", sd.color.w, sd.font.m.handle);
    if (!timeError) {
        DrawFormatStringToHandle(sd.screen.right() - 250, sd.obj.upperFrame.box.bottom() + 10,
            sd.color.w, sd.font.m.handle, "%4d/%02d/%02d(%3s) %02d:%02d:%02d",
